...
export function ConnectedPanel({ neighbors }: Props) {
  ...
  return (
    <aside className="rounded-lg border border-zinc-800 bg-zinc-900 p-4 space-y-3">
        ...
        {neighbors.map((n) => {
          ...
          return (
            <li key={n.id}>
              <Link
                href={`/t/${n.id}`}
                className="block rounded-md border border-zinc-800 p-2 hover:border-zinc-700"
              >
